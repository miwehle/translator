...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, ff_dim: int, dropout: float, attention: str):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, src_key_padding_mask: torch.Tensor) -> torch.Tensor:
        y = self.norm1(x)
        attn_out, _ = self.self_attn(y, y, y, key_padding_mask=src_key_padding_mask, need_weights=False)
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        self_attn_out, _ = self.self_attn(
            y, y, y, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask, need_weights=False
        )
        x = x + self.drop1(self_attn_out)

        y = self.norm2(x)
        cross_attn_out, _ = self.cross_attn(
            y, memory, memory, key_padding_mask=src_key_padding_mask, need_weights=False
        )
