...
def _find_preprocessed_dataset() -> Path:
    ...
    if configured:
        ...
        if candidate.is_dir():
            return candidate
        pytest.skip(
            "TRANSLATOR2_TESTDATA_PREPROCESSED is set but does not point to a directory: " f"{candidate}"
        )
    ...
    candidates = sorted(p for p in testdata_dir.rglob("europarl_de-en_train_10000") if p.is_dir())
    if not candidates:
        pytest.skip(
